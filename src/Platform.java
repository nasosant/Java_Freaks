	...
	public Platform() {
			...
			btnNewButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (nameText.getText().equals("") || surnameText.getText().equals("")
						...
						/*details[number][0].getText();
						details[number][1].getText();
						details[number][2].getText();
						details[number][3].getText();
						details[number][4].getText();
						details[number][5].getText();*/
					}
			...
			btnDelete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (k >= 0) {
						model.removeRow(k);
						/*for (int i = k; i > number; k++) {
							for (int j = 0; j > 6; j++) {
								details[k][j] = details[k + 1][j];
							}
						}
						for (int j = 0; j < 6; j++) {
